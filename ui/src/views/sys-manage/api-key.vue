<template>
  <div class="public-block">
    ...
    <el-dialog
      :visible.sync="showKeyDialogVisible"
      :modal-append-to-body="false"
      :close-on-click-modal="false"
      :close-on-press-escape="false"
      width="600px"
      top="15vh"
      custom-class="success-key-dialog"
    >
      <div slot="title" class="success-dialog-title">
        <i
          class="el-icon-success"
          style="color: #67C23A; font-size: 24px; margin-right: 10px; vertical-align: middle;"
        />
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listApiKey({ projectName: this.projectName })
        .then((response) => {
          if (response.data) {
    ...
    handleUpdate(row) {
      ...
      getApiKey({ id: apiKeyId, projectName: this.projectName }).then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除该API密钥？', '提示', {
        ...
        .then(() => {
          return deleteApiKey({ id: apiKeyId, projectName: this.projectName })
        })
        .then(() => {
          this.getList()
