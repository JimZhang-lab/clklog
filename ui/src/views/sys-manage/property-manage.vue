    ...
    <div class="documentation-container">
      <el-form :inline="true" size="mini" :model="query">
        <el-form-item label="属性">
          <el-input v-model="query.keyword" clearable placeholder="属性名/显示名" @keyup.enter.native="loadList" />
        </el-form-item>
    ...
    <div class="public-block">
      <div class="block-main public-hoverItem">
        <div class="table-head">
          <div class="public-tableHead">属性管理</div>
          <el-button class="zc_btn_default" size="mini" icon="el-icon-plus" @click="openAdd">新增属性</el-button>
        </div>
        <el-table
          v-loading="loading"
          border
        ...
        >
          ...
          <el-table-column prop="propertyName" label="属性名" min-width="160" show-overflow-tooltip />
          ...
          <el-table-column label="通用" width="90">
            <template slot-scope="scope">{{ scope.row.commonProperty ? "是" : "否" }}</template>
          </el-table-column>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="560px" :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="90px" size="mini">
        <el-form-item label="属性名" prop="propertyName">
          <el-input v-model="form.propertyName" placeholder="如 plan_id / $screen_name" />
        </el-form-item>
        ...
        <el-form-item label="分组">
          <el-input v-model="form.groupName" placeholder="如 用户属性 / 事件属性" />
        </el-form-item>
        <el-form-item label="标签">
          <el-input v-model="form.tag" placeholder="如 系统、业务、渠道" />
        </el-form-item>
        <el-form-item label="通用属性">
          <el-switch v-model="form.commonProperty" />
        </el-form-item>
...
import {
  ...
  getPageListApi,
  getTagListApi,
} from "@/api/sysmanage/manageAttribute";
...
export default {
  ...
  methods: {
    ...
    loadOptions() {
      ...
      getGroupNameListApi(params).then((res) => (this.groupOptions = res.data || []));
      getTagListApi(params).then((res) => (this.tagOptions = res.data || []));
    },
    loadList() {
      this.loading = true;
      const params = Object.assign({ projectName: this.projectName }, this.query, {
    ...
    submit() {
      this.$refs.form.validate((valid) => {
        ...
        const request = payload.id ? editApi(payload) : addApi(payload);
        request.then(() => {
    ...
    remove(row) {
      this.$confirm(`确认删除 ${row.displayName || row.propertyName} 吗？`, "提示", {
        type: "warning",
      }).then(() => {
