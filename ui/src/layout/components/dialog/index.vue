    ...
    >
      <div class="el-dialog-div" v-if="!refreshFlag">
        <div class="left-item">
          ...
          <div class="userTags commonBackgrond public-hoverItem">
            <div class="tag-header">
              <span class="common-title">用户标签</span>
              <el-popover placement="bottom-end" width="320" trigger="click">
                <el-form size="mini" label-width="70px">
                  <el-form-item label="选择标签">
                    <el-select v-model="assignTagId" filterable placeholder="请选择">
                      <el-option
                  ...
                  <el-form-item label="标签值">
                    <el-input v-model="assignTagValue" placeholder="可填写具体分层值" />
                  </el-form-item>
                  <el-form-item>
                    <el-button type="primary" :loading="tagLoading" @click="assignUserTag">
                      确定
                    </el-button>
                  </el-form-item>
                </el-form>
                <el-button slot="reference" type="text" icon="el-icon-plus">添加标签</el-button>
              </el-popover>
            </div>
            <div v-loading="tagLoading" class="tag-list">
              <el-tag
                ...
                closable
                size="small"
                @close="removeUserTag(item)"
              >
                {{ item.displayName }}：{{ item.tagValue || item.displayName }}
              </el-tag>
              <span v-if="!tagLoading && !userTags.length" class="empty-tag">暂无标签</span>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    assignUserTag() {
      ...
      const tag = this.tagOptions.find((item) => item.id === this.assignTagId);
      this.tagLoading = true;
      assignTagApi({
        projectName: this.projectName,
        tagId: this.assignTagId,
        distinctId: this.distinctId,
        tagValue: this.assignTagValue || (tag && tag.displayName) || "",
      })
        .then(() => {
    ...
    getVisitorDetailinfo() {
      let params = { distinctId: this.distinctId, projectName: this.projectName };
    ...
    getVisitorSessionUriList(val) {
      ...
      getVisitorSessionUriListApi(params).then((res) => {
        if (res.code == 200) {
          this.loading = false;
          this.visitListUrl = (res.data && res.data.rows) || [];
          if (this.visitListUrl.length === 0) {
            return;
          }
