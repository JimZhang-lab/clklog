...
import { getChannelListApi } from "@/api/trackingapi/channel";
export default {
  components: {
    ErrorLog,
    Screenfull,
      ...
      handler: function (newVal, oldVal) {
        if (newVal && newVal.length > 0) {
          ...
          if (!this.prejectCode || !newVal.some((item) => item.projectName === this.prejectCode)) {
            ...
            this.loadChannelList(this.prejectCode);
          }
        } else {
  ...
  methods: {
    ...
    handleChangeProject(val) {
      ...
      this.loadChannelList(val);
    },
    loadChannelList(projectName) {
      if (!projectName) {
        this.$store.dispatch("user/setChannelList", []);
        return;
      }
      getChannelListApi({ projectName })
        .then((res) => {
          if (res.code === 200) {
            this.$store.dispatch("user/setChannelList", res.data || []);
          }
        })
        .catch(() => {
          this.$store.dispatch("user/setChannelList", []);
